...
impl ProjectPanel {
                                ...
                                .detach_and_prompt_err("打开文件失败", window, cx, move |e, _, _| {
                                    match e.error_code() {
                                        ErrorCode::Disconnected => if is_via_ssh {
                                            Some("已断开与 SSH 主机的连接".to_string())
                                        } else {
                                            Some("已断开与远程项目的连接".to_string())
                                        },
                ...
                menu.context(self.focus_handle.clone()).map(|menu| {
                    if is_read_only {
                        menu.when(is_markdown, |menu| {
                            menu.action("打开 Markdown 预览", Box::new(OpenMarkdownPreview))
                        })
                        .when(is_dir, |menu| {
                        ...
                        menu.action("新建文件", Box::new(NewFile))
                            ...
                            .when(!is_root && !is_collab, |menu| {
                                menu.action("移入回收站", Box::new(Trash { skip_prompt: false }))
                            })
                            .when(!is_root, |menu| {
    ...
    fn restore_file(
        ...
        maybe!({
            ...
            let answer = if !action.skip_prompt {
                let prompt = format!("放弃对 {} 的更改？", MarkdownInlineCode(&file_name));
        ...
        maybe!({
            ...
            cx.spawn(async move |_, cx| {
                if let Err(e) = receiver.await? {
                    if let Some(workspace) = workspace.upgrade() {
                        cx.update(|cx| {
                            let message = format!("添加到 .git/info/exclude 失败：{}", e);
                            let toast = StatusToast::new(message, cx, |this, _| {
                                this.icon(Icon::new(IconName::XCircle).color(Color::Error))
                                    .dismiss_button(true)
                            });
        ...
        S: AsRef<str>,
    {
        ...
        let mut message = match names {
            ...
            _ => {
                ...
                if omitted_count == 1 {
                    listed_names.push(".. 1 个文件未显示".into());
                } else if omitted_count > 1 {
                    listed_names.push(format!(".. {omitted_count} 个文件未显示"));
                }
        ...
        match dirty_buffers {
            0 => {}
            1 if names.len() == 1 => {
                message.push_str("它有未保存的更改，这些更改将丢失。");
            }
            1 => {
                message.push_str("其中 1 个有未保存的更改，这些更改将丢失。");
            }
            dirty_buffers => {
                message.push_str(&format!(
                    "其中有 {dirty_buffers} 个存在未保存的更改，这些更改将丢失。"
                ));
            }
        }
    ...
    fn show_remove_failure_toast(
        ...
        let message = match (trash, total_count) {
            (true, 1) => format!("有 {failed_count}/{total_count} 个文件移入回收站失败。"),
            (true, _) => format!("有 {failed_count}/{total_count} 个文件移入回收站失败。"),
            (false, 1) => format!("有 {failed_count}/{total_count} 个文件删除失败。"),
            (false, _) => format!("有 {failed_count}/{total_count} 个文件删除失败。"),
        };
