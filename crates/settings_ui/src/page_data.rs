            ...
            SettingsPageItem::DynamicItem(
                DynamicItem {
                    discriminant: SettingItem {
                        files: USER,
                        title: "按钮布局",
                        description: "（仅 Linux）选择标题栏中窗口控制按钮的布局方式。",
                        field: Box::new(SettingField {
                            organization_override: None,
...
fn ai_page(cx: &App) -> SettingsPage {
    fn general_section() -> [SettingsPageItem; 8] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                title: "会话侧边栏默认宽度",
                description: "会话侧边栏的默认宽度。修改此项也会更新已手动调整过大小的侧边栏，双击分隔条可重置为该宽度。",
                field: Box::new(SettingField {
                    organization_override: None,
            ...
            SettingsPageItem::SubPageLink(SubPageLink {
                ...
                description: Some(
                    "查看、添加和移除通过 Agent Client Protocol 连接的智能体。".into(),
                ),
                search_aliases: &[
                    "acp",
            ...
            SettingsPageItem::SubPageLink(SubPageLink {
                ...
                description: Some("查看、添加、配置和移除 Model Context Protocol 服务器。".into()),
    ...
    fn agent_configuration_section(_cx: &App) -> Box<[SettingsPageItem]> {
        ...
        items.extend([
            ...
            SettingsPageItem::SubPageLink(SubPageLink {
                ...
                description: Some("查看并更改始终无需提示即可允许的提权终端沙箱权限。".into()),
                search_aliases: &[
                    "allow",
                    "domain",
                    "filesystem",
            ...
            SettingsPageItem::SubPageLink(SubPageLink {
                ...
                description: Some(
                    "设置正则表达式模式，以针对特定工具输入自动允许、自动拒绝或始终请求确认。"
                        .into(),
                ),
                search_aliases: &[],
                in_json: true,
...
fn non_editor_language_settings_data() -> Box<[SettingsPageItem]> {
    fn lsp_section() -> [SettingsPageItem; 10] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                title: "联动编辑",
                description: "在语言服务器支持时，是否对关联区间执行联动编辑。例如编辑开头的 <html> 标签时，结尾的 </html> 标签内容也会一并修改。",
                field: Box::new(SettingField {
                    organization_override: None,
