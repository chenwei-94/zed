        ...
        serde_json::json!({
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "tcp_connection": {
                        "type": "object",
                        "description": "用于连接已在运行的调试适配器的可选 TCP 连接信息",
                        "properties": {
                            "port": {
                                "type": "integer",
                                "description": "调试适配器监听的端口（默认：自动查找可用端口）"
                            },
                            "host": {
                                "type": "string",
                                "description": "调试适配器监听的主机，形式为 IPv4 或 IPv6 地址（默认：127.0.0.1）"
                            },
                            "timeout": {
                                "type": "integer",
                                "description": "连接 TCP DAP 并返回错误前的最大毫秒数（默认：2000ms）"
                            }
                        }
                    }
                },
